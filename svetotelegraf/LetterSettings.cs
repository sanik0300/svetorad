...
{
    /// <summary>
    /// класс для настройки частоты и способов сигнализирования
    /// </summary>
    class LetterSettings
    {
        ...
        /// <summary>
        /// Вычисляем длительность 1 отрезка согласно частоте
        /// </summary>
        ...
        /// <summary>
        /// сохраняет просьбу пользователя орать в случае звонка
        /// </summary>
        public bool OriginallyBeeps { get; private set; }
        public LetterSettings(string txt, int hz, bool beeeep)
        {
            Txt = txt;
            OriginallyBeeps = beeeep;
            OnPlay = true;
            interval = (int)Math.Round(60000 /(AvgLetterLen*hz));
        }
        public uint position = 0;

        public void Un_Mute(bool wat) { OriginallyBeeps = wat;}
    }
}
